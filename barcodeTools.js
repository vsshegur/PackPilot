...
export function encodeCode128B(value) {
  ...
  const startCode = 104;
  const dataCodes = [...text].map(character => character.charCodeAt(0) - 32);
  ...
  return {
    ...
    codes,
    patterns,
    moduleCount: patterns.reduce((sum, pattern) => sum + [...pattern].reduce((width, digit) => width + Number(digit), 0), 0)
  };
}
